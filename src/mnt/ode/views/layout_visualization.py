...
import numpy as np
from PyQt6.QtCore import QByteArray, QEvent, QSize, Qt, pyqtSignal, pyqtSlot
...
class LayoutVisualizationWidget(QWidget):  # type: ignore[misc]
    ...
    def set_layout_figures(
        ...
        self._distance_pixmaps = self._figures_to_pixmaps(distance_figures)
        self._presence_pixmaps = self._figures_to_pixmaps(presence_figures)
    ...
    def update_active_pixmaps(self) -> None:
        ...
        if num_plots > 0:
            ...
            if 2**self._num_input_pairs != num_plots:
                ...
                self._num_input_pairs = max(0, int(np.log2(num_plots)))
    ...
    def display_figure(self, figure: Figure | None) -> None:
        """Display a specific figure in the pixmap label.

        Args:
            figure: The figure to display. If None, the pixmap label is cleared.
        """
        logger.debug("Received request to display a specific figure.")
        if figure:
            svg_bytes = self._figure_to_svg_bytes(figure)
            pixmap = self._svg_bytes_to_pixmap(svg_bytes)
            self.pixmap_label.setPixmap(self._scaled_pixmap(pixmap))
            logger.info("Specific figure displayed as pixmap.")
        else:
            self.pixmap_label.clear()
            logger.warning("Request to display a None figure. Clearing pixmap label.")
    ...
    def _figures_to_pixmaps(self, figures: Sequence[Figure | None]) -> list[QPixmap]:
        """Convert a sequence of matplotlib Figures to QPixmap objects.

        Args:
            figures: A sequence of matplotlib Figures to convert.

        Returns:
            The converted QPixmap objects.
        """
        pixmaps: list[QPixmap] = []
        for fig in figures:
            if fig is not None:
                svg_bytes = self._figure_to_svg_bytes(fig)
                pixmap = self._svg_bytes_to_pixmap(svg_bytes)
