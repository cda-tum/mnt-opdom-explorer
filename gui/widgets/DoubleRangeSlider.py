...
"""
Taken from https://github.com/djeada/Qt-Widgets/blob/master/src/widgets/python/double_range_slider/double_range_slider.py
"""
...
class DoubleRangeSlider(QSlider):
    ...
    def paintEvent(self, event):
        """
        Paint the slider along with custom tick marks and handles.
        """
    ...
    def _draw_ticks(self, painter):
        """
        Draw smaller, refined tick marks for the slider.
        """
        tick_interval = 100  # This corresponds to an interval of 1.0 in float range (scale_factor = 100)
        tick_length = 4  # Shorter tick marks (in pixels) for a subtler look
        pen = QPen(Qt.GlobalColor.gray, 1)  # Thinner, lighter-colored pen for ticks
        painter.setPen(pen)

        # Draw ticks across the slider width at regular intervals
        num_ticks = int(self.maximum() / tick_interval)
        for i in range(num_ticks + 1):
            x_pos = int(i * self.width() / num_ticks)  # Cast to int
            painter.drawLine(x_pos, self.height() - tick_length, x_pos, self.height())

    def _draw_handle(self, option, painter, style, x, state):
        if self.orientation() == Qt.Orientation.Horizontal:
            option.rect = QRect(int(x), 0, HANDLE_WIDTH, self.height())
        else:
            option.rect = QRect(0, int(x), self.width(), HANDLE_WIDTH)
